			 	...
			 	uintptr_t x_vtc_ptr)
{
	...
	vtc_Config = XVtc_LookupConfig(bAddress);
	if(vtc_Config == NULL)
	{
		printf("ERR:: VTC device not found\r\n");
		return(XST_DEVICE_NOT_FOUND);
	}
	vtc_Config->BaseAddress = x_vtc_ptr;
	Status = XVtc_CfgInitialize(x_vtc, vtc_Config, vtc_Config->BaseAddress);
	if(Status != XST_SUCCESS)
	{
		printf("ERR:: VTC Initialization failed %d\r\n", Status);
		return(XST_FAILURE);
	}
	printf("VTC: Initialized OK\r\n");
    return(XST_SUCCESS);
}
/*--------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------*/
