...
extern int XConfigInterruptCntrl(uint32_t IntcParent);
extern int XConnectToInterruptCntrl(uint32_t IntrId, void *IntrHandler, void *CallBackRef,
				    uint32_t IntcParent);
#if defined (__MICROBLAZE__) || defined(__riscv)
extern int XConnectToFastInterruptCntrl(uint32_t IntrId, void *IntrHandler, uint32_t IntcParent);
#endif
extern int XDisconnectInterruptCntrl(uint32_t IntrId, uint32_t IntcParent);
extern int XStartInterruptCntrl(uint32_t Mode, uint32_t IntcParent);
extern void XEnableIntrId( uint32_t IntrId, uint32_t IntcParent);
extern void XDisableIntrId( uint32_t IntrId, uint32_t IntcParent);
#if defined (XPAR_SCUGIC)
extern void XSetPriorityTriggerType( uint32_t IntrId, uint8_t Priority, uint32_t IntcParent);
extern void XGetPriorityTriggerType( uint32_t IntrId, uint8_t *Priority, uint8_t *Trigger, uint32_t IntcParent);
#endif
extern void XStopInterruptCntrl( uint32_t IntcParent);
extern void XRegisterInterruptHandler( void *IntrHandler, uint32_t IntcParent);
extern int XSetupInterruptSystem(void *DriverInstance, void *IntrHandler, uint32_t IntrId,
				 uint32_t IntcParent,
				 uint16_t Priority);
extern uint32_t XGetEncodedIntrId(uint32_t LegacyIntrId, uint32_t TriggerType, uint8_t IntrType, uint8_t IntcType,
			     uint32_t *IntrId);
extern uint32_t XTriggerSoftwareIntr(uint32_t IntrId, uint32_t IntcParent, uint32_t Cpu_Id);
#endif
