...
uint32_t XGet_Zynq_UltraMp_Platform_info(void)
{
	...
	return (uint32_t)((reg.Arg1 >> XPLAT_INFO_SHIFT) & XPLAT_INFO_MASK);
#else
	uint32_t reg;
...
uint32_t XGetPSVersion_Info(void)
{
	...
	XSmc_OutVar reg;
	reg = Xil_Smc(GET_CHIPID_SMC_FID, 0, 0, 0, 0, 0, 0, 0);
	return (uint32_t)((reg.Arg1 &  XPS_VERSION_INFO_MASK) >>
...
#else
	{
		register uint32_t C15Reg __asm(XREG_CP15_MULTI_PROC_AFFINITY);
		CoreId = C15Reg;
	}
	CoreId &= XREG_MPIDR_MASK;
#endif
...
uint8_t XGetBootStatus(void)
{
	...
	uint8_t ClusterId = XGetClusterId();
