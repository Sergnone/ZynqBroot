...
#if (EL3 == 1)
#define mfelrel3() ({uint64_t rval = 0U; \
		__asm volatile("mrs %0,  ELR_EL3" : "=r" (rval));\
		rval;\
	})
#define mfesrel3() ({uint64_t rval = 0U; \
		__asm volatile("mrs %0,  ESR_EL3" : "=r" (rval));\
		rval;\
	})

#define mffarel3() ({uint64_t rval = 0U; \
		__asm volatile("mrs %0,  FAR_EL3" : "=r" (rval));\
		rval;\
	})

#else
#define mfelrel1() ({uint64_t rval = 0U; \
		__asm volatile("mrs %0,  ELR_EL1" : "=r" (rval));\
		rval;\
	})

#define mfesrel1() ({uint64_t rval = 0U; \
		__asm volatile("mrs %0,  ESR_EL1" : "=r" (rval));\
		rval;\
	})

#define mffarel1() ({uint64_t rval = 0U; \
...
/* CP15 operations */
#define mfcp(reg)	({uint64_t rval = 0U;\
		__asm__ __volatile__("mrs	%0, " #reg : "=r" (rval));\
		rval;\
	})

#define mfcpnotoken(reg)       ({uint64_t rval = 0U;\
		__asm__ __volatile__("mrs       %0, " reg : "=r" (rval));\
		rval;\
	})
