...
#define XENV_MEM_FILL(DestPtr, Data, Bytes) \
	memset((void *) DestPtr, (uint32_t) Data, (size_t) Bytes)
 ...
 */
typedef uint32_t XENV_TIME_STAMP;
...
#  define XCACHE_INVALIDATE_DCACHE_RANGE(Addr, Len) \
			microblaze_invalidate_dcache_range((uint32_t)(Addr), (uint32_t)(Len))

#if (XPAR_MICROBLAZE_DCACHE_USE_WRITEBACK == 1)
#  define XCACHE_FLUSH_DCACHE()  		microblaze_flush_dcache()
#  define XCACHE_FLUSH_DCACHE_RANGE(Addr, Len) \
			microblaze_flush_dcache_range((uint32_t)(Addr), (uint32_t)(Len))
#else
...
#  define XCACHE_INVALIDATE_ICACHE_RANGE(Addr, Len) \
			microblaze_invalidate_icache_range((uint32_t)(Addr), (uint32_t)(Len))
...
#define XCACHE_INVALIDATE_DCACHE_RANGE(Addr, Len) \
		XCache_InvalidateDCacheRange((uint32_t)(Addr), (uint32_t)(Len))

#define XCACHE_FLUSH_DCACHE_RANGE(Addr, Len) \
		XCache_FlushDCacheRange((uint32_t)(Addr), (uint32_t)(Len))
