...
(XScuGic_WriteReg(InstancePtr->RedistBaseAddr, RegOffset, (uint32_t)Data))
...
#define XScuGic_Enable_SystemReg_CPU_Interface_EL3() mtcpnotoken(XREG_ICC_SRE_EL3, (uint64_t)0xF);
#define XScuGic_Enable_SystemReg_CPU_Interface_EL1() mtcpnotoken(XREG_ICC_SRE_EL1, (uint64_t)0xF);
#elif defined (ARMR52)
...
#else
#define XScuGic_Enable_Group0_Interrupts() mtcpnotoken(XREG_ICC_IGRPEN0_EL1,(uint64_t)0x1);
#endif
/****************************************************************************/
/**
...
#else
#define XScuGic_set_priority_filter(val)  mtcpnotoken(XREG_ICC_PMR_EL1, (uint64_t)val)
#endif
/****************************************************************************/
/**
...
#elif EL3
#define XScuGic_ack_Int(val)   mtcpnotoken(XREG_ICC_EOIR0_EL1,(uint64_t)val)
#else
...
uint32_t  XScuGic_Connect(XScuGic *InstancePtr, uint32_t Int_Id,
...
uint32_t  XScuGic_CfgInitialize(XScuGic *InstancePtr, XScuGic_Config *ConfigPtr,
							uint32_t EffectiveAddr);

uint32_t  XScuGic_SoftwareIntr(XScuGic *InstancePtr, uint32_t Int_Id, uint32_t Cpu_Identifier);
...
#ifndef SDT
uint8_t XScuGic_IsInitialized(uint32_t DeviceId);
#else
 ...
 */
XScuGic_Config *XScuGic_LookupConfig(uint16_t DeviceId);
/*
 * Lookup configuration by using BaseAddress
 */
XScuGic_Config *XScuGic_LookupConfigBaseAddr(uint32_t BaseAddress);
#else
XScuGic_Config *XScuGic_LookupConfig(uint32_t BaseAddr);
#endif
 ...
 */
uint32_t  XScuGic_SelfTest(XScuGic *InstancePtr);
