...
#include "xvidc.h"
#include "../../include/xbasic_types.h"
...
static uint8_t XVidC_IsVtmRb(const char *VideoModeStr, uint8_t RbN);
 ...
 *
*******************************************************************************/
uint32_t XVidC_RegisterCustomTimingModes(const XVidC_VideoTimingMode *CustomTable,
		                            uint16_t NumElems)
{
	uint16_t Index;
 ...
 *
*******************************************************************************/
uint64_t XVidC_GetPixelClockHzByHVFr(uint32_t HTotal, uint32_t VTotal, uint8_t FrameRate)
{
 ...
 *
*******************************************************************************/
uint64_t XVidC_GetPixelClockHzByVmId(XVidC_VideoMode VmId)
{
	uint64_t ClkHz;
 ...
 *
*******************************************************************************/
uint8_t XVidC_IsInterlaced(XVidC_VideoMode VmId)
{
 ...
 *
*******************************************************************************/
XVidC_VideoMode XVidC_GetVideoModeId(uint32_t Width, uint32_t Height, uint32_t FrameRate,
 ...
 *
*******************************************************************************/
XVidC_VideoMode XVidC_GetVideoModeIdRb(uint32_t Width, uint32_t Height,
 ...
 *
*******************************************************************************/
uint32_t XVidC_SetVideoStream(XVidC_VideoStream *VidStrmPtr, XVidC_VideoMode VmId,
 ...
 *
*******************************************************************************/
uint32_t XVidC_Set3DVideoStream(XVidC_VideoStream *VidStrmPtr, XVidC_VideoMode VmId,
			   XVidC_ColorFormat ColorFormat, XVidC_ColorDepth Bpc,
...
void XVidC_ReportStreamInfo(const XVidC_VideoStream *Stream)
{
	if (!XVidC_GetVideoModeData(Stream->VmId) &&
		...
		printf("\tThe stream ID (%d) is not supported.\r\n",
				Stream->VmId);
		return;
	}
	...
	if (Stream->VmId == XVIDC_VM_CUSTOM) {
		...
		if (Stream->IsDSCompressed) {
			printf("\tResolution[Compressed]:   %dx%d [Custom Mode]\r\n",
				   Stream->Timing.HActive,
				   Stream->Timing.VActive);
			printf("\tResolution[Uncompressed]: %dx%d\r\n",
				   Stream->UncompressedTiming.HActive,
				   Stream->UncompressedTiming.VActive);
			printf("\tPixel Clock[Compressed]:  %u KHz\r\n",
				   (uint32_t)XVidC_GetPixelClockHzByHVFr(Stream->Timing.HTotal,
								    Stream->Timing.F0PVTotal,
								    Stream->FrameRate) / 1000);
		} else {
			printf("\tResolution:               %dx%d [Custom Mode]\r\n",
				   Stream->Timing.HActive, Stream->Timing.VActive);
			printf("\tPixel Clock:              %d kHz\r\n",
				   (uint32_t)XVidC_GetPixelClockHzByHVFr(
		...
		printf("\tPixel Clock:              %d kHz\r\n",
				(uint32_t)XVidC_GetPixelClockHzByVmId(Stream->VmId)/1000);
	}
}
 ...
 *
*******************************************************************************/
void XVidC_ReportTiming(const XVidC_VideoTiming *Timing, uint8_t IsInterlaced)
{
	...
	/* Interlaced */
	if (IsInterlaced) {
		printf("\tVSYNC Timing (Field 0): vav=%04d, vfp=%02d, "
	...
	printf("\tVSYNC Timing (Field 1): vav=%04d, vfp=%02d, "
			"vsw=%02d(vsp=%d), vbp=%03d, vtot=%04d\r\n",
	...
	/* Progressive */
		...
		printf("\tVSYNC Timing: vav=%04d, vfp=%02d, "
			"vsw=%02d(vsp=%d), vbp=%03d, vtot=%04d\r\n",
			Timing->VActive, Timing->F0PVFrontPorch,
			Timing->F0PVSyncWidth, Timing->VSyncPolarity,
			Timing->F0PVBackPorch, Timing->F0PVTotal);
