...
* 5.2   ml   09/07/23 Typecasting with uint32_t to fix MISRA-C_RULE_10.3 violation.
...
static uint32_t CpuId; /**< CPU Core identifier */
...
*
******************************************************************************/
int32_t  XScuGic_CfgInitialize(XScuGic *InstancePtr,
...
*
****************************************************************************/
int32_t  XScuGic_Connect(XScuGic *InstancePtr, uint32_t Int_Id,
...
void XScuGic_Disconnect(XScuGic *InstancePtr, uint32_t Int_Id)
{
	 ...
	 */
	Mask = (uint32_t)0x00000001U << (Int_Id % 32U);
	 ...
	 */
	XScuGic_DistWriteReg(InstancePtr, (uint32_t)XSCUGIC_DISABLE_OFFSET +
			     ((Int_Id / 32U) * 4U), Mask);
...
{
	uint32_t Mask;
	uint8_t Cpu_Identifier = (uint8_t)CpuId;
	 ...
	 */
	XScuGic_DistWriteReg(InstancePtr, (uint32_t)XSCUGIC_ENABLE_SET_OFFSET +
			     ((Int_Id / 32U) * 4U), Mask);
...
*
******************************************************************************/
int32_t  XScuGic_SoftwareIntr(XScuGic *InstancePtr, uint32_t Int_Id, uint32_t Cpu_Identifier)
{
#if defined (GICv3)
	uint64_t Mask;
#else
...
*
*****************************************************************************/
void XScuGic_SetPriorityTriggerType(XScuGic *InstancePtr, uint32_t Int_Id,
				    uint8_t Priority, uint8_t Trigger)
{
	...
	Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);
	Xil_AssertVoid(Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS);
	Xil_AssertVoid(Trigger <= (uint8_t)XSCUGIC_INT_CFG_MASK);
	Xil_AssertVoid(LocalPriority <= (uint8_t)XSCUGIC_MAX_INTR_PRIO_VAL);
#if defined (GICv3)
	if (Int_Id < XSCUGIC_SPI_INT_ID_START ) {
	 ...
	 * register
	 */
	RegValue &= ~((uint32_t)XSCUGIC_PRIORITY_MASK << ((Int_Id % 4U) * 8U));
	RegValue |= (uint32_t)LocalPriority << ((Int_Id % 4U) * 8U);
	 ...
	 * register
	 */
	RegValue &= ~((uint32_t)XSCUGIC_INT_CFG_MASK << ((Int_Id % 16U) * 2U));
	RegValue |= (uint32_t)Trigger << ((Int_Id % 16U) * 2U);
				    ...
				    uint8_t *Priority, uint8_t *Trigger)
{
	...
	if (Int_Id < XSCUGIC_SPI_INT_ID_START ) {
		...
		*Trigger = (uint8_t)(RegValue & XSCUGIC_INT_CFG_MASK);
		return;
	}
#endif
	/*
	 ...
	 * are preserved.
	 */
	*Priority = (uint8_t)(RegValue & XSCUGIC_PRIORITY_MASK);
...
*
*****************************************************************************/
void XScuGic_InterruptMaptoCpu(XScuGic *InstancePtr, uint8_t Cpu_Identifier, uint32_t Int_Id)
{
	uint32_t RegValue;
	...
	if (Int_Id >= XSCUGIC_SPI_INT_ID_START) {
		...
		RegValue |= (uint32_t)(Cpu_CoreId) << (Offset * 8U);
		XScuGic_DistWriteReg(InstancePtr,
				     XSCUGIC_SPI_TARGET_OFFSET_CALC(Int_Id),
				     RegValue);
		/*
...
*
*****************************************************************************/
void XScuGic_InterruptUnmapFromCpu(XScuGic *InstancePtr, uint8_t Cpu_Identifier, uint32_t Int_Id)
{
/*
 * For GICv3 1 of N SPI interrupt selection mode is not supported in
 * driver. Specific interrupt can be routed to only specific core.
 ...
 */
	...
	if (Int_Id >= XSCUGIC_SPI_INT_ID_START) {
		...
		Offset = (Int_Id & 0x3U);
		Cpu_CoreId = ((uint32_t)0x1U << Cpu_Identifier);
...
*
*****************************************************************************/
void XScuGic_UnmapAllInterruptsFromCpu(XScuGic *InstancePtr, uint8_t Cpu_Identifier)
{
/*
 * For GICv3 1 of N SPI interrupt selection mode is not supported in
 * driver. Specific interrupt can be routed to only specific core.
 ...
 */
	...
	uint32_t Int_Id;
	uint32_t Target_Cpu;
	uint32_t LocalCpuID = ((uint32_t)1U << Cpu_Identifier);
...
void XScuGic_Stop(XScuGic *InstancePtr)
{
	...
	uint32_t DistDisable = 0; /* Do not disable distributor */
#else
	uint32_t DistDisable = 1; /* Track distributor status*/
#endif
	uint32_t LocalCpuID = ((uint32_t)0x1 << CpuId);
	...
	LocalCpuID |= ((uint32_t)XGetCoreId() << XSCUGIC_IROUTER_AFFINITY1_SHIFT);
		...
		if ((Target_Cpu != LocalCpuID)) {
			 ...
			 */
			 XScuGic_DistWriteReg(InstancePtr,XSCUGIC_DISABLE_OFFSET +
                         (((Int_Id) / 32U) * 4U), ((uint32_t)0x00000001U << ((Int_Id) % 32U)));
	...
	for (Int_Id = 32U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS;
		...
		Target_Cpu = XScuGic_DistReadReg(InstancePtr,
						 XSCUGIC_SPI_TARGET_OFFSET_CALC(Int_Id));
		if ((Target_Cpu != LocalCpuID) && (Target_Cpu != (uint32_t)0)) {
			/*
...
*
*****************************************************************************/
uint8_t XScuGic_IsInitialized(uint32_t BaseAddress)
{
	uint8_t Device_Initilaized = 0U;
