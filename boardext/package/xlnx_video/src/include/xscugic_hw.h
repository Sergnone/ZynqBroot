...
* 5.5   ml   01/08/25 Update datatype of function arguments from uint32_t to uint32_t to
...
*
* @note  C-style signature:
*        uint32_t XScuGic_ReadReg64(uint32_t BaseAddress, uint32_t RegOffset)
*
*****************************************************************************/
...
uint32_t  XScuGic_DeviceInitialize(uint32_t DeviceId);
void XScuGic_RegisterHandler(uint32_t BaseAddress, uint32_t InterruptID,
			     Xil_InterruptHandler IntrHandler, void *CallBackRef);
void XScuGic_SetPriTrigTypeByDistAddr(uint32_t DistBaseAddress, uint32_t Int_Id,
                                        uint8_t Priority, uint8_t Trigger);
void XScuGic_GetPriTrigTypeByDistAddr(uint32_t DistBaseAddress, uint32_t Int_Id,
					uint8_t *Priority, uint8_t *Trigger);
void XScuGic_InterruptMapFromCpuByDistAddr(uint32_t DistBaseAddress,
							uint8_t Cpu_Id, uint32_t Int_Id);
void XScuGic_InterruptUnmapFromCpuByDistAddr(uint32_t DistBaseAddress,
											uint8_t Cpu_Id, uint32_t Int_Id);
void XScuGic_UnmapAllInterruptsFromCpuByDistAddr(uint32_t DistBaseAddress,
												uint8_t Cpu_Id);
void XScuGic_EnableIntr (uint32_t DistBaseAddress, uint32_t Int_Id);
void XScuGic_DisableIntr (uint32_t DistBaseAddress, uint32_t Int_Id);
#if defined(GICv3)
uint32_t XScuGic_GetRedistBaseAddr(void);
#endif
