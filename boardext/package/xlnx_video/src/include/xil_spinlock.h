...
#if defined(__GNUC__)
/************************** Function Prototypes *****************************/
uint32_t Xil_SpinLock(void);
uint32_t Xil_SpinUnlock(void);
uint32_t Xil_InitializeSpinLock(uint32_t lockaddr, uint32_t lockflagaddr,
		                                                    uint32_t lockflag);
void Xil_ReleaseSpinLock(void);
uint32_t Xil_IsSpinLockEnabled(void);
...
/**< protect multiple applications running at separate CPUs to write to
  ...
  */
#define XIL_SPINLOCK()              \
    if(Xil_IsSpinLockEnabled()!=(uint32_t)0) {    \
        Xil_SpinLock();  }
#else
#define XIL_SPINLOCK()
#endif /* (__GNUC__) */
