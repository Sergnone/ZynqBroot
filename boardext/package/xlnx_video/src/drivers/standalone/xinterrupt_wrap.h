...
#ifdef XIL_INTERRUPT
#include "../../include/xcommon_drv_config.h"
...
extern int XConfigInterruptCntrl(uintptr_t IntcParent);
extern int XConnectToInterruptCntrl(uint32_t IntrId, void *IntrHandler, void *CallBackRef,
				    uintptr_t IntcParent);
#if defined (__MICROBLAZE__) || defined(__riscv)
extern int XConnectToFastInterruptCntrl(uint32_t IntrId, void *IntrHandler, uintptr_t IntcParent);
#endif
extern int XDisconnectInterruptCntrl(uint32_t IntrId, uintptr_t IntcParent);
extern int XStartInterruptCntrl(uint32_t Mode, uintptr_t IntcParent);
extern void XEnableIntrId( uint32_t IntrId, uintptr_t IntcParent);
extern void XDisableIntrId( uint32_t IntrId, uintptr_t IntcParent);
#if defined (XPAR_SCUGIC)
extern void XSetPriorityTriggerType( uint32_t IntrId, uint8_t Priority, uintptr_t IntcParent);
extern void XGetPriorityTriggerType( uint32_t IntrId, uint8_t *Priority, uint8_t *Trigger, uintptr_t IntcParent);
#endif
extern void XStopInterruptCntrl( uintptr_t IntcParent);
extern void XRegisterInterruptHandler( void *IntrHandler, uintptr_t IntcParent);
extern int XSetupInterruptSystem(void *DriverInstance, void *IntrHandler, uint32_t IntrId,
				 uintptr_t IntcParent,
				 uint16_t Priority);
extern int32 XGetEncodedIntrId(uint32_t LegacyIntrId, uint32_t TriggerType, uint8_t IntrType, uint8_t IntcType,
			     uint32_t *IntrId);
extern int32 XTriggerSoftwareIntr(uint32_t IntrId, uintptr_t IntcParent, uint32_t Cpu_Id);
#endif
