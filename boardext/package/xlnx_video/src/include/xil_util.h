...
*			  API from uint32_t to uint32_t for supporting 64 bit addressing.
...
*      ml        09/13/23 Replaced numerical types (int) with proper typedefs(uint32_t) to
...
uint32_t Xil_Strcpy(char *DestPtr, const char *SrcPtr, const uint32_t Size);

/**< Copies specified range from source string to destination string */
uint32_t Xil_StrCpyRange(const uint8_t *Src, uint8_t *Dest, uint32_t From, uint32_t To, uint32_t MaxSrcLen,
		    uint32_t MaxDstLen);

/**< Appends string2 to string1 */
uint32_t Xil_Strcat(char *Str1Ptr, const char *Str2Ptr, const uint32_t Size);

/**< Compares Len bytes from memory1 and memory2 */
uint32_t Xil_MemCmp(const void *Buf1Ptr, const void *Buf2Ptr, uint32_t Len);
