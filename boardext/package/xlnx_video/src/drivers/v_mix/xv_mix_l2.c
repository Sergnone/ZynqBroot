...
* 2.00  rco   07/21/16   Used uintptr_t instead of uint32_t for Baseaddress
...
static const uint8_t bkgndColorYUV[XVMIX_BKGND_LAST][3] =
{
  {  0, 128, 128}, //Black
  {255, 128, 128}, //White
  { 76,  85, 255}, //Red
...
static const uint8_t bkgndColorRGB[XVMIX_BKGND_LAST][3] =
{
  {0, 0, 0}, //Black
  {1, 1, 1}, //White
  {1, 0, 0}, //Red
                    ...
                    uintptr_t remapAddress)
{
  ...
  Status = XV_mix_Initialize(&InstancePtr->Mix,
                            BaseAddress,
                            remapAddress);
                         ...
                         XVMix_Scalefactor Scale)
{
  ...
  uint16_t ScaleFactor[XVMIX_SCALE_FACTOR_NUM_SUPPORTED] = {1,2,4};
...
{
  XV_mix *MixPtr;
  uint32_t NumLayers, CurrenState;
...
{
  uint16_t v_b_val,y_r_val,u_g_val;
  uint16_t scale;
       ...
       if(LayerId < XVMix_GetNumLayers(InstancePtr)) {
         ...
         if(WinResInRange) {
           ...
           if(WinValid) {
             ...
             uint32_t BaseStartXReg, BaseStartYReg;
             uint32_t BaseWidthReg, BaseHeightReg;
             uint32_t BaseStrideReg;
             uint32_t Offset;
...
{
  XV_mix *MixPtr;
  uintptr_t BaseReg, Align;
  uint32_t WinValid = FALSE;
  int Status = XST_FAILURE;
...
*
******************************************************************************/
uintptr_t XVMix_GetLayerBufferAddr(XV_Mix_l2 *InstancePtr, XVMix_LayerId LayerId)
{
  XV_mix *MixPtr;
  uint32_t BaseReg;
  uintptr_t ReadVal = 0;
...
*
******************************************************************************/
uintptr_t XVMix_GetLayerChromaBufferAddr(XV_Mix_l2 *InstancePtr,
                                       XVMix_LayerId LayerId)
{
  XV_mix *MixPtr;
  uint32_t BaseReg;
 ...
 *                      Must be greater than XVMIX_LAYER_MASTER and less than XVMIX_LAYER_LOGO.
 *
 * @return The address (uintptr_t) of the second chroma buffer for the specified layer.
 ...
 */
uintptr_t XVMix_GetLayerChromaBuffer2Addr(XV_Mix_l2 *InstancePtr,
                                       XVMix_LayerId LayerId)
{
  XV_mix *MixPtr;
  uint32_t BaseReg;
                   ...
                   uint8_t *BBuffer)
{
  XV_mix *MixPtr;
  uint32_t x,y;
  uint32_t Rword, Gword, Bword;
  uint32_t Width, Height;
  uint32_t RBaseAddr, GBaseAddr, BBaseAddr;
  int Status = XST_FAILURE;
  ...
  if(XVMix_IsLogoEnabled(InstancePtr)) {
      ...
      for (y=0; y<Height; y++) {
          for (x=0; x<Width; x+=4) {
            Rword = (uint32_t)RBuffer[y*Width+x] |
                    (((uint32_t)RBuffer[y*Width+x+1])<<8) |
                    (((uint32_t)RBuffer[y*Width+x+2])<<16) |
                    (((uint32_t)RBuffer[y*Width+x+3])<<24);

            Gword = (uint32_t)GBuffer[y*Width+x] |
                    (((uint32_t)GBuffer[y*Width+x+1])<<8) |
                    (((uint32_t)GBuffer[y*Width+x+2])<<16) |
                    (((uint32_t)GBuffer[y*Width+x+3])<<24);

            Bword = (uint32_t)BBuffer[y*Width+x] |
                    (((uint32_t)BBuffer[y*Width+x+1])<<8) |
                    (((uint32_t)BBuffer[y*Width+x+2])<<16) |
                    (((uint32_t)BBuffer[y*Width+x+3])<<24);
                             ...
                             uint8_t *ABuffer)
{
  ...
  if(XVMix_IsLogoEnabled(InstancePtr) &&
     XVMix_IsLogoPixAlphaEnabled(InstancePtr)) {
      ...
      for (y=0; y<Height; y++) {
          for (x=0; x<Width; x+=4) {
            Aword = (uint32_t)ABuffer[y*Width+x] |
                    (((uint32_t)ABuffer[y*Width+x+1])<<8) |
                    (((uint32_t)ABuffer[y*Width+x+2])<<16) |
                    (((uint32_t)ABuffer[y*Width+x+3])<<24);
 ...
 * to the standard output using printf. The reported information includes:
 *   - Pixels per clock
 *   - Color depth
 *   - Number of layers
 ...
 *
 * The function uses printf for output and is intended for debugging purposes.
...
void XVMix_DbgLayerInfo(XV_Mix_l2 *InstancePtr, XVMix_LayerId LayerId)
{
  ...
  switch(LayerId) {
    ...
    default: //Layer1-8
        ...
        printf("Type : %s\r\n", IntfType[LayerType]);
        if(IsEnabled) {
          uint32_t Stride, Reg, Offset;

          printf("Addr : 0x%x\r\n",
                       XVMix_GetLayerBufferAddr(InstancePtr, LayerId));
          printf("Chroma Addr : 0x%x\r\n",
 ...
 *
 ***/
void XVMix_SetCoeffForYuvToRgb(XV_Mix_l2 *InstancePtr,
		XVidC_ColorStd ColorStd,
		XVidC_ColorRange colorRange,
		uint8_t colorDepth)
{
	XV_mix *MixPtr;
	uint32_t scale_factor = 1 << XVMIX_CSC_COEFF_FRACTIONAL_BITS;
	uint32_t bpcScale = 1 << (colorDepth-8);
	uint32_t i;
 ...
 */
void XVMix_SetCoeffForRgbToYuv(XV_Mix_l2 *InstancePtr,
		XVidC_ColorStd ColorStd,
		XVidC_ColorRange colorRange,
		uint8_t colorDepth)
{
 ...
 */
uint32_t XVMix_SetCscCoeffs(XV_Mix_l2 *InstancePtr,
