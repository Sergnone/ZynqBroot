...
* 5.5   ml   01/08/25 Update datatype of function arguments from uint32_t to uintptr_t to
...
#endif
static XScuGic_Config *LookupConfigByBaseAddress(uintptr_t CpuBaseAddress);
...
*
******************************************************************************/
int32_t XScuGic_DeviceInitialize(uint32_t DistBaseAddr)
{
...
void XScuGic_DeviceInterruptHandler(void *DeviceId)
{
	...
	CfgPtr = LookupConfigByBaseAddress((uintptr_t)DeviceId);
...
*
******************************************************************************/
void XScuGic_RegisterHandler(uintptr_t BaseAddress, int32_t InterruptID,
...
*
*****************************************************************************/
void XScuGic_SetPriTrigTypeByDistAddr(uintptr_t DistBaseAddress, uint32_t Int_Id,
				      uint8_t Priority, uint8_t Trigger)
{
	...
	uintptr_t RedistBaseAddr;
#endif
	uint8_t LocalPriority = Priority;
...
*
*****************************************************************************/
void XScuGic_GetPriTrigTypeByDistAddr(uintptr_t DistBaseAddress, uint32_t Int_Id,
				      uint8_t *Priority, uint8_t *Trigger)
{
	uint32_t RegValue;
#if defined (GICv3)
...
*
*****************************************************************************/
void XScuGic_InterruptMapFromCpuByDistAddr(uintptr_t DistBaseAddress,
	uint8_t Cpu_Id, uint32_t Int_Id)
{
	uint32_t RegValue;
#if !defined (GICv3)
	uint32_t Offset;
...
*
*****************************************************************************/
void XScuGic_InterruptUnmapFromCpuByDistAddr(uintptr_t DistBaseAddress,
	uint8_t Cpu_Id, uint32_t Int_Id)
{
/*
 ...
 */
	...
	if (Int_Id >= XSCUGIC_SPI_INT_ID_START) {
		...
		Offset = (Int_Id & 0x3U);
		Cpu_CoreId = ((uint32_t)0x1U << Cpu_Id);
...
*
*****************************************************************************/
void XScuGic_UnmapAllInterruptsFromCpuByDistAddr(uintptr_t DistBaseAddress,
	uint8_t Cpu_Id)
{
/*
 * For GICv3 1 of N SPI interrupt selection mode is not supported in
 ...
 */
	...
	uint32_t Int_Id;
	uint32_t Target_Cpu;
	uint32_t LocalCpuID = ((uint32_t)1U << Cpu_Id);
...
*
****************************************************************************/
void XScuGic_EnableIntr (uintptr_t DistBaseAddress, uint32_t Int_Id)
{
	uint8_t Cpu_Id = (uint8_t)XScuGic_GetCpuID();
#if defined (GICv3)
	uint32_t Temp;
	uintptr_t RedistBaseAddr;
#endif
...
*
****************************************************************************/
void XScuGic_DisableIntr (uintptr_t DistBaseAddress, uint32_t Int_Id)
{
#if ! defined(GICv3)
	uint8_t Cpu_Id = (uint8_t)XScuGic_GetCpuID();
#endif
...
*
******************************************************************************/
uintptr_t XScuGic_GetRedistBaseAddr(void)
{
	uint32_t Cpu_Affinity;
	uint64_t Gicr_Typer;
	uintptr_t BaseAddr = 0, Addr;
