...
void XGpioPs_IntrEnablePin(XGpioPs *InstancePtr, uint32_t Pin)
{
	...
	IntrReg = ((uint32_t)1 << (uint32_t)PinNumber);
	XGpioPs_WriteReg(InstancePtr->GpioConfig.BaseAddr,
			  ((uint32_t)(Bank) * XGPIOPS_REG_MASK_OFFSET) +
...
uint32_t XGpioPs_IntrGetEnabledPin(const XGpioPs *InstancePtr, uint32_t Pin)
{
	...
	return (((IntrReg & ((uint32_t)1 << PinNumber)) != (uint32_t)0)? (uint32_t)FALSE : (uint32_t)TRUE);
}
...
uint32_t XGpioPs_IntrGetStatusPin(const XGpioPs *InstancePtr, uint32_t Pin)
{
	...
	return (((IntrReg & ((uint32_t)1 << PinNumber)) != (uint32_t)0)? (uint32_t)TRUE : (uint32_t)FALSE);
}
...
{
	uint32_t IntrTypeReg;
	uint32_t IntrPolReg;
	uint32_t IntrOnAnyReg;
	uint8_t Bank;
	uint8_t PinNumber;
	...
	switch (IrqType) {
		case XGPIOPS_IRQ_TYPE_EDGE_RISING:
			IntrTypeReg |= ((uint32_t)1 << (uint32_t)PinNumber);
			IntrPolReg |= ((uint32_t)1 << (uint32_t)PinNumber);
			IntrOnAnyReg &= ~((uint32_t)1 << (uint32_t)PinNumber);
			break;
...
uint8_t XGpioPs_GetIntrTypePin(const XGpioPs *InstancePtr, uint32_t Pin)
{
	...
	IntrType = XGpioPs_ReadReg(InstancePtr->GpioConfig.BaseAddr,
				    ((uint32_t)(Bank) * XGPIOPS_REG_MASK_OFFSET) +
				    XGPIOPS_INTTYPE_OFFSET) & ((uint32_t)1 << PinNumber);

	if (IntrType == ((uint32_t)1 << PinNumber)) {

		IntrOnAny = XGpioPs_ReadReg(InstancePtr->GpioConfig.BaseAddr,
				     ((uint32_t)(Bank) * XGPIOPS_REG_MASK_OFFSET) +
				     XGPIOPS_INTANY_OFFSET) & ((uint32_t)1 << PinNumber);

		IntrPol = XGpioPs_ReadReg(InstancePtr->GpioConfig.BaseAddr,
				   ((uint32_t)(Bank) * XGPIOPS_REG_MASK_OFFSET) +
				   XGPIOPS_INTPOL_OFFSET) & ((uint32_t)1 << PinNumber);


		if (IntrOnAny == ((uint32_t)1 << PinNumber)) {
			IrqType = XGPIOPS_IRQ_TYPE_EDGE_BOTH;
		} else if (IntrPol == ((uint32_t)1 << PinNumber)) {
		...
		if (IntrPol == ((uint32_t)1 << PinNumber)) {
