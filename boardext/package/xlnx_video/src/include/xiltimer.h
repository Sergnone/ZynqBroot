...
typedef void (*XTimer_TickHandler) (void *CallBackRef, uint32_t StatusEvent);
...
typedef struct XTimerTag {
	void (*XTimer_ModifyInterval)(struct XTimerTag *InstancePtr, uint32_t delay,
               XTimer_DelayType Delaytype); /**<Modifies the timer interval*/
	void (*XTimer_TickIntrHandler)(struct XTimerTag *InstancePtr,
               uint8_t Priority);                /**< Tick interrupt handler */
	void (*XTimer_TickInterval)(struct XTimerTag *InstancePtr, uint32_t Delay);
...
uint32_t XTimer_GetLibVersion(void)
{
 ...
 */
uint32_t XilSleepTimer_Init(XTimer *InstancePtr);
/**
 * This API is used for initializing Tick timer
 */
uint32_t XilTickTimer_Init(XTimer *InstancePtr);
/**
 * Get the time
 */
...
uint32_t Xil_GetMBFrequency(void);
uint32_t Xil_SetMBFrequency(uint32_t Val);
#endif

#ifdef XTIMER_DEFAULT_TIMER_IS_MB_RISCV
uint32_t Xil_GetRISCVFrequency(void);
uint32_t Xil_SetRISCVFrequency(uint32_t Val);
#endif
