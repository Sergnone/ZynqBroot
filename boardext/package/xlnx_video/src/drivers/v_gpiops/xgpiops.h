...
* 3.14  bkv  02/20/25 Changed data type of effective address from uint32_t to uintptr_t
...
#include "../standalone/xplatform_info.h"
...
#define XGPIOPS_BANK_MAX_PINS		(uint32_t)32 /**< Max pins in a GPIO bank */
...
#define XGPIOPS_DEVICE_MAX_PIN_NUM_ZYNQMP	(uint32_t)174 /**< Max pins in the
						  *	Zynq Ultrascale+ MP GPIO device
					      * 0 - 25,  Bank 0
					      * 26 - 51, Bank 1
					      *	52 - 77, Bank 2
					      ...
					      *	142 - 173, Bank 5
					      */
#define XGPIOPS_DEVICE_MAX_PIN_NUM	(uint32_t)118 /**< Max pins in the Zynq GPIO device
					      * 0 - 31,  Bank 0
					      * 32 - 53, Bank 1
					      *	54 - 85, Bank 2
					      *	86 - 117, Bank 3
 ...
 *
 *****************************************************************************/
typedef void (*XGpioPs_Handler) (void *CallBackRef, uint32_t Bank, uint32_t Status);
...
typedef struct {
	char *Name;
	uintptr_t BaseAddr;		/**< Register base address */
	uint16_t IntrId;		/** Bits[11:0] Interrupt-id Bits[15:12]
				 * trigger type and level flags */
	uintptr_t IntrParent; 	/** Bit[0] Interrupt parent type Bit[64/32:1]
				 * Parent base address */
} XGpioPs_Config;
...
typedef struct {
	...
	uint8_t MaxBanks;			/**< Max banks in a GPIO device */
    uint32_t PmcGpio;                    /**< Flag for accessing PS GPIO for versal*/
	uint32_t CoreIntrMask[XGPIOPS_MAX_BANKS_CNT]; /**< Interrupt mask per core */
} XGpioPs;
...
int32 XGpioPs_SelfTest(XGpioPs *InstancePtr);

/* Functions in xgpiops_intr.c */
/* Bank APIs in xgpiops_intr.c */
void XGpioPs_IntrEnable(XGpioPs *InstancePtr, uint8_t Bank, uint32_t Mask);
void XGpioPs_IntrDisable(XGpioPs *InstancePtr, uint8_t Bank, uint32_t Mask);
uint32_t XGpioPs_IntrGetEnabled(const XGpioPs *InstancePtr, uint8_t Bank);
uint32_t XGpioPs_IntrGetStatus(const XGpioPs *InstancePtr, uint8_t Bank);
void XGpioPs_IntrClear(const XGpioPs *InstancePtr, uint8_t Bank, uint32_t Mask);
void XGpioPs_SetIntrType(const XGpioPs *InstancePtr, uint8_t Bank, uint32_t IntrType,
			  uint32_t IntrPolarity, uint32_t IntrOnAny);
void XGpioPs_GetIntrType(const XGpioPs *InstancePtr, uint8_t Bank, uint32_t *IntrType,
			  uint32_t *IntrPolarity, uint32_t *IntrOnAny);
...
void XGpioPs_SetIntrTypePin(const XGpioPs *InstancePtr, uint32_t Pin, uint8_t IrqType);
uint8_t XGpioPs_GetIntrTypePin(const XGpioPs *InstancePtr, uint32_t Pin);

void XGpioPs_IntrEnablePin(XGpioPs *InstancePtr, uint32_t Pin);
void XGpioPs_IntrDisablePin(XGpioPs *InstancePtr, uint32_t Pin);
uint32_t XGpioPs_IntrGetEnabledPin(const XGpioPs *InstancePtr, uint32_t Pin);
uint32_t XGpioPs_IntrGetStatusPin(const XGpioPs *InstancePtr, uint32_t Pin);
void XGpioPs_IntrClearPin(const XGpioPs *InstancePtr, uint32_t Pin);

/* Functions in xgpiops_sinit.c */
XGpioPs_Config *XGpioPs_LookupConfig(uint32_t BaseAddress);
#ifdef __cplusplus
}
#endif
