...
*			   Users can always use printfs if they want to debug.
...
* 2.3   ms     01/23/17 Modified printf statement in main function for all
...
typedef struct {
	...
	int DmaStatus;			/**< 0 on success, otherwise error code
					 */
	uint32_t ChanFaultType;	/**< Channel fault type in case of fault
				 */
	uint32_t ChanFaultPCAddr;	/**< Channel fault PC address
				 */
} XDmaPs_Cmd;
...
static INLINE int XDmaPs_Instr_DMAMOV(char *DmaProg, unsigned Rd, uint32_t Imm);
