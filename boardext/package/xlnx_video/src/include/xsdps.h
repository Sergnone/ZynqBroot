...
typedef struct {
	...
	uint32_t OTapDly_DDR_Clk50;	/**< Output Tap delay for DDR50 modes */
	uint32_t OTapDly_SDR_Clk100;	/**< Input Tap delay for SDR50 modes */
	uint32_t OTapDly_SDR_Clk200;	/**< Input Tap delay for SDR104/HS200 modes */
	uint32_t OTapDly_DDR_Clk200;	/**< Input Tap delay for HS400 modes */
} XSdPs_Config;
...
typedef struct {
	...
	uint8_t  IsTuningDone;	/**< Flag to indicate HS200 tuning complete */
...
uint32_t XSdPs_ReadPolled(XSdPs *InstancePtr, uint32_t Arg, uint32_t BlkCnt, uint8_t *Buff);
uint32_t XSdPs_WritePolled(XSdPs *InstancePtr, uint32_t Arg, uint32_t BlkCnt, const uint8_t *Buff);
uint32_t XSdPs_Idle(XSdPs *InstancePtr);

uint32_t XSdPs_Change_BusSpeed(XSdPs *InstancePtr);
uint32_t XSdPs_Change_ClkFreq(XSdPs *InstancePtr, uint32_t SelFreq);
uint32_t XSdPs_Pullup(XSdPs *InstancePtr);
uint32_t XSdPs_Get_BusWidth(XSdPs *InstancePtr, uint8_t *ReadBuff);
uint32_t XSdPs_Change_BusWidth(XSdPs *InstancePtr);
uint32_t XSdPs_Get_BusSpeed(XSdPs *InstancePtr, uint8_t *ReadBuff);
uint32_t XSdPs_Get_Mmc_ExtCsd(XSdPs *InstancePtr, uint8_t *ReadBuff);
uint32_t XSdPs_Set_Mmc_ExtCsd(XSdPs *InstancePtr, uint32_t Arg);
uint32_t XSdPs_SetBlkSize(XSdPs *InstancePtr, uint16_t BlkSize);
uint32_t XSdPs_Get_Status(XSdPs *InstancePtr, uint8_t *SdStatReg);
uint32_t XSdPs_Select_Card(XSdPs *InstancePtr);
uint32_t XSdPs_StartReadTransfer(XSdPs *InstancePtr, uint32_t Arg, uint32_t BlkCnt, uint8_t *Buff);
uint32_t XSdPs_CheckReadTransfer(XSdPs *InstancePtr);
uint32_t XSdPs_StartWriteTransfer(XSdPs *InstancePtr, uint32_t Arg, uint32_t BlkCnt, uint8_t *Buff);
uint32_t XSdPs_CheckWriteTransfer(XSdPs *InstancePtr);
uint32_t XSdPs_Erase(XSdPs *InstancePtr, uint32_t StartAddr, uint32_t EndAddr);
uint32_t XSdPs_Sanitize(XSdPs *InstancePtr);
